...
public class SimulateurTestTP3 {
	...
	*/
	...
	//dans les trois fonctions diffrentes le TEB pour un SNR sera toujours de 0. Nous ne pouvons tester que cette valeur
	// car nous savons  l'avance le TEB qui correspond  ce SNR.
	// Les tests suivants vont nous permettre de vrifier que mme avec du bruit pour un SNR de 0 on aura un TEB de 0
	...
	public void TestNRZBruite() throws Exception{
		...
		String options2[] = {"-mess","0101010101010101111111","-form","NRZ", "- snr 0"};
	...
	public void TestNRZTBruite() throws Exception{
		...
		String options2[] = {"-mess","0101010101010101111111","-form","NRZT", "- snr 0"};
		simulateur = new Simulateur(options2);
		simulateur.execute();
		collector.checkThat(simulateur.calculTauxErreurBinaire(),is((float)0));
		
		// Avec message predefini et sonde  puis codage NRZT
		String options3[] = {"-mess","0101001010101010101111111","-form","NRZT","-s", "- snr 0"};
		simulateur = new Simulateur(options3);
		simulateur.execute();
		collector.checkThat(simulateur.calculTauxErreurBinaire(),is((float)0));
		
		// Avec message aleatoire  puis codage NRZT
		String options4[] = {"-mess","48","-form","NRZT", "- snr 0"};
		simulateur = new Simulateur(options4);
		simulateur.execute();
		collector.checkThat(simulateur.calculTauxErreurBinaire(),is((float)0));
		
		// Avec message aleatoire et sonde  puis codage NRZ
		String options5[] = {"-mess","88","-form","NRZT","-s", "- snr 0"};
		simulateur = new Simulateur(options5);
		simulateur.execute();
		collector.checkThat(simulateur.calculTauxErreurBinaire(),is((float)0));
		
		// Avec message aleatoire et seed  puis codage NRZT
		String options6[] = {"-mess","42","-form","NRZT","-seed","1", "- snr 0"};
		simulateur = new Simulateur(options6);
		simulateur.execute();
		collector.checkThat(simulateur.calculTauxErreurBinaire(),is((float)0));
	...
	public void TestRZBruite() throws Exception{
		...
		String options7[] = {"-seed","1","-form","NRZT", "- snr 0"};
		simulateur = new Simulateur(options7);
		simulateur.execute();
